...
**The three armatures committed 2026-08-30 — what actually happened.** Measured
2026-08-30; the three files do **not** behave alike:

| File | Placements | Recorded seed rebuilds | Orphans |
|---|---|---|---|
| `dominant-seed669141` | 28 | 24 marks | **4** (ids 24–27) |
| `sketch-b-seed28960` | 27 | 25 marks | **2** (ids 26–27) |
| `triangle-seed669141` | 28 | 25 marks | **3** (ids 25–27) |

**The desync spanned armature and hero type as well as seed.** The editor's
placement store was initialised once from the app's **startup plan — `dominant`
at seed 1** — and never rebuilt as the seed, the armature or the curation
controls changed. So in each file the recorded `seed` / `armature` / `curation`
describe the state at **export**, while the placements describe the state at
**store init**. Those are different moments, and nothing reconciled them.

The clearest illustration is `triangle-seed669141`: it is labelled `"armature":
"triangle"` but **22 of its 28 placements sit at `dominant`-at-seed-1 positions**.
The file carries one armature's figures under another's name. `dominant-seed669141`
is the same story with 23 of 28 at startup positions. Against their own recorded
seed and armature, all three match **zero** positions.

`sketch-b-seed28960` matches the startup plan only weakly (6 of 27); it passed
through intermediate states that are **not reconstructable**, and no attempt is
made here to say which.

**No mark count was ever inconsistent** — every armature/seed pair rebuilds
deterministically to the count shown above. The files simply pair one plan's
placements with another plan's label. They pass `planSignature` because their
recorded seed and armature still rebuild correctly today, which is precisely the
limit of that guard: it catches plan-*code* drift, not a file whose placements
never belonged to the plan it names.
