<!--
  p5.brush-reference.md — CODE-AGENT REFERENCE (vendored)
  ============================================================================
  This file = a PROJECT OVERLAY (below) + the UPSTREAM p5.brush API doc
  (verbatim, further down). Read the overlay FIRST.

  Authority order: SPEC.md > PATTERNS.md > this overlay > upstream body.
  Where the upstream body disagrees with PATTERNS/SPEC, PATTERNS/SPEC win —
  they are tested against our pinned beta; the upstream doc tracks @latest.

  Re-upload to Claude.ai project knowledge alongside SPEC/PATTERNS so both
  seats see it. Replace the upstream body wholesale on a p5.brush version bump;
  don't edit it in place.
  ============================================================================
-->

# p5.brush — Project Reference (overlay)

## Our reality (read before trusting any snippet below)

- **Build: p5 build only.** WEBGL canvas, p5.js 2.x. We do **not** use the
  standalone build in current work. (Standalone is filed as *future R&D* — see
  "Future capabilities" — ignore it for anything shipping now.)
- **Version: pinned `p5.brush@2.1.9-beta`, p5 `^2.2`.** The upstream body's
  install line uses `p5.brush@latest` / `p5@2.2` — **that is not us.** Some
  `@latest` API surface may not exist (or may differ) in 2.1.9-beta. **Verify any
  API the upstream body describes actually exists in the pinned beta before
  relying on it.**
- **Install specifics (from PATTERNS, tested):**
  - Dist path is **`dist/p5.brush.js`** (UMD). There is **no `p5.brush.min.js`** in
    the npm package — jsdelivr only serves that path by auto-minifying on the fly,
    so it breaks against a real local install / stricter host.
  - Behind a bundler, use **instance mode** (`new p5(sketch)` + `brush.instance(p)`).
    Global-mode auto-detection does not fire reliably when served behind a bundler.

## Overrides & traps vs the upstream body

- **`brush.spline()` is NOT a spine primitive for our ribbons.** Its stamp engine
  drags the tip *parallel* to the path, so it can only make strokes **along** the
  spine — it cannot place the **perpendicular comb-teeth** a Ribbon needs. That is
  precisely why station placement (Catmull-Rom + `placeStations`, teeth along the
  normal) is **our** code, not the library's (SPEC §core, brush-lab adapter). The
  upstream "smooth curve through points" framing will mislead if taken at face
  value. Same reason the SVG-spine importer flattens to a polyline and feeds
  `placeStations` rather than using `brush.spline` (PATTERNS → SVG spine import).
- **Seeding.** p5 build: `randomSeed(n)` / `noiseSeed(n)` seed the brush lib
  automatically (good). The V1.0 append-only reveal adds a project-specific rule on
  top: **per-mark `randomSeed(seed + i)`** before drawing mark *i*, so a mark looks
  identical whether drawn in the full static pass or one-at-a-time during the reveal
  (PATTERNS → draw-on / append-only). Not optional for the reveal loop.
- **Custom brushes are already how we tune material.** `brush.add()` (with the full
  param surface — `noise`, `pressure`, `tip`, `grain`, `sharpness`) is the same
  mechanism behind our `rebuildBuiltinBrush` / `rebuildCustomBrush` (built-in
  vibration override via `BUILTIN_DNA`, since `brush.set` has no vibration channel).
  The upstream `brush.add()` table documents that surface — treat it as reference for
  what we already do, not a new path.

## Project runtime constraints that pair with this API (not in the upstream doc)

- **`pixelDensity(2)` once at top of `setup()`** — never toggle at runtime.
- **Never `resizeCanvas` a p5.brush WEBGL canvas at runtime** — corrupts p5.brush's
  internal framebuffer (block/checkerboard tearing until reload). Size once; window
  resize ⇒ reload.
- **Append-only reveal** uses `setAttributes('preserveDrawingBuffer', true)` once in
  setup, paint ground once, never `background()` again (PATTERNS → draw-on approach C).
- **Translucency is compositional**, never per-mark alpha (SPEC). Watercolor
  `fill`/`fillBleed`/`wash` are largely out of our dry-media lane — noted, not used.

## Already in use vs future capabilities

- **In use:** `brush.set` / `brush.line` (teeth), `brush.beginShape`/`vertex`/
  `endShape` (Linework spine stroke), `brush.field('hand')` (Bend Field),
  `brush.add` (custom/built-in rebuild), `brush.box`, `scaleBrushes`.
- **Future generator material (not yet used):** `brush.mass()` — dry-media
  hand-filled hatched fill (crayon/pastel), the most on-lineage thing here for
  *filled* Enfantines areas; `brush.addField()` — bespoke vector fields for future
  Bend-Field work; `hatch` / `hatchArray` / `massArray` on polygons — for the
  shape-based generators still on the roadmap (Bloom, Fan, Field Marks).
- **Future R&D (whole build):** the **standalone build** (no p5 dependency, browser
  WebGL2, manual `brush.render()` / `brush.clear()` frame control) may give a cleaner
  compositing lifecycle for a future audio-reactive piece — worth a spike someday,
  out of scope for V1.

---

*Everything below is the upstream p5.brush reference (Alejandro Campos Uribe),
vendored verbatim. Trust the overlay above it on any conflict.*
...
- `brush.massArray(polygons)` — Apply current mass style to a `brush.Polygon` or array of `brush.Polygon` objects as one combined gesture (array = outer shapes + holes via even-odd logic).
...
- `brush.spline(points, curvature?)` — Smooth curve through `[[x,y], [x,y,pressure], ...]`. Curvature 0–1. Returns `brush.Plot` for reuse.
...
- `brush.rect(x, y, w, h, mode?)` — `mode`: `"corner"` (default) or `"center"`. Returns nothing.
- `brush.circle(x, y, radius, r?)` — `r` = hand-drawn irregularity 0–1. Returns `[plot, offsetX, offsetY]` for reuse.
- `brush.arc(x, y, radius, start, end)` — Stroke only. Angles follow current `angleMode()`. Returns `brush.Plot`, or `null` on zero sweep.
- `brush.polygon(pointsArray)` — Polygon from `[[x,y], ...]`. Not affected by vector fields. Returns `brush.Polygon` for reuse.
- `brush.beginShape(curvature?)` / `brush.vertex(x, y, pressure?)` / `brush.endShape(close?)` — `endShape()` returns `brush.Plot` for reuse.
...
Primitives return their geometry — store the return value to apply effects later or reuse the shape:

```js
const frame = brush.polygon([[50,50],[350,50],[350,350],[50,350]])  // returns Polygon
brush.hatch(8, Math.PI / 4)
frame.hatch()              // hatch the stored polygon

brush.mass("pastel", "#4b6cb7", { strength: 0.8 })
frame.mass()               // mass the same geometry
```

For multiple polygons as one gesture (even-odd hole logic), use `brush.hatchArray(polygons)` or `brush.massArray(polygons)`.
