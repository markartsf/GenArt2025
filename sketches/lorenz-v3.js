...
export class LorenzAttractorV3 {
  ...
  draw(audioFeatures) {
    ...
    // V3: Fade based on aesthetic (slower in country, faster in urban)
    this.ctx.fillStyle = `rgba(26, 10, 10, ${this.aesthetic.current.trailFade})`;
    this.ctx.fillRect(0, 0, width, height);

    // Lorenz parameters respond to aesthetic + subtle audio influences
    // Smooth transitions, no pulsing
    const tempoInfluence = this.aesthetic.current.speed;

    // Subtle parameter drift based on audio features (not reactive pulsing)
    // High frequencies gently influence sigma
    const sigmaInfluence = audioFeatures.spectralCentroid / 5000;
    this.sigma = 10 * (1 + tempoInfluence * 2 + sigmaInfluence * 0.3);

    // Mid frequencies gently influence rho
    const rhoInfluence = audioFeatures.mid * 0.5;
    this.rho = 28 * (1 + tempoInfluence * 0.5 + rhoInfluence * 0.2);

    // Bass gently influences beta
    const betaInfluence = audioFeatures.bass * 0.3;
    this.beta = (8/3) * (1 + tempoInfluence * 0.3 + betaInfluence * 0.15);

    // Rotation speed matches aesthetic (country = slow, urban = fast)
    // Plus subtle micro-variations from high frequencies
    const rotationSpeed = 0.003 + (this.aesthetic.current.speed * 0.015) + (audioFeatures.high * 0.002);
    this.rotation += rotationSpeed;

    // Scale based on aesthetic
    const dynamicScale = this.scale + (this.aesthetic.current.speed * 20);

    // Trail length varies with audio intensity (longer trails with more energy)
    // This creates visual density changes without pulsation
    this.maxPoints = Math.floor(this.baseMaxPoints * (0.7 + audioFeatures.rms * 0.6));
    ...
    for (let i = 1; i < this.points.length; i++) {
      ...
      // V3: MUCH THICKER LINES - 15-30px range
      // Varies with aesthetic (thicker in country, slightly thinner in urban)
      const baseThickness = this.aesthetic.current.lineThickness;

      // Gentle variation along path (not pulsing)
      const pathVariation = Math.sin(i * 0.05) * 4;

      // Very subtle audio influence (not reactive pulsing)
