...
class PersistentBeam {
  constructor(width, height, instrumentType, colorIndex) {
    ...
    // CRITICAL: Trail NEVER clears, just keeps growing
    this.trail = [];
  ...
  update(audioFeatures, aesthetic) {
    ...
    // NEVER clear the trail, just cap it for performance
    this.trail.push({ x: this.x, y: this.y });

    if (this.trail.length > this.maxTrailPoints) {
      this.trail.shift(); // Only remove oldest if over cap
    }
  }
  ...
  draw(ctx, aesthetic) {
    ...
    // Draw entire trail as continuous path
    ...
    const lastPoint = this.trail[trailLength - 1];
...
export class LongExposureBeamsV3 {
  ...
  draw(audioFeatures) {
    ...
    const avgTrailLength = this.beams.reduce((sum, b) => sum + b.trail.length, 0) / this.beams.length;
