...
class ParticleV3 {
  ...
  draw(ctx, aesthetic) {
    ...
    // Draw trail as continuous line with gradient opacity
    for (let i = 1; i < this.trail.length; i++) {
      ...
      const thickness = 2 + (trailProgress * 4);
      ctx.lineWidth = thickness;
      ctx.lineCap = 'round';
      ctx.lineJoin = 'round';

      // Glow effect in urban areas or on transient bursts
      if (aesthetic.current.speed > 0.6 || aesthetic.transientBurst > 0.3) {
        const glowAmount = 8 + (aesthetic.current.speed * 15) + (aesthetic.transientBurst * 20);
