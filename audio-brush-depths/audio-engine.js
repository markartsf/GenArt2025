...
class AudioEngine {
  constructor() {
    ...
    // Beat intensity (0-1, how STRONG the beat was)
    this.beatStrength = 0;

    // Beat density (beats per second, rolling window)
    this.recentBeats = [];    // timestamps of recent beats
    this.beatDensity = 0;     // beats/sec over last 2s
  ...
  loadFile(file) {
    ...
    // The existing source node (created in _ensureContext) automatically follows
    // the audio element's new src — no need to recreate it.
    this.audioEl.addEventListener('canplaythrough', () => {
      this.isReady = true;
      document.getElementById('playBtn').disabled = false;
    ...
    // Also listen for errors on the new source
    this.audioEl.addEventListener('error', () => {
      document.getElementById('status').textContent = 'Failed to load ' + file.name;
      document.getElementById('status').className = 'error';
    }, { once: true });
  }
  ...
  analyze() {
    ...
    if ((beatByRMS || beatByFlux) && this.beatCooldown <= 0) {
      // Beat strength: how far above threshold (normalized 0-1)
      const rmsDelta = rawRMS - this.avgRMS;
      const fluxStrength = this.spectralFlux / Math.max(FLUX_THRESHOLD * 3, 0.01);
      this.beatStrength = Math.min(1, Math.max(
        rmsDelta / 0.06,          // RMS delta 0.06 = full strength
        fluxStrength               // flux normalized to threshold×3
      ));
      ...
      // Beat density: track timestamps, count beats in last 2s
      this.recentBeats.push(now);
    }
    this.beatFlash *= 0.82;
    this.beatCooldown = Math.max(0, this.beatCooldown - 1);

    // Update beat density (beats per second over last 2s window)
    const densityNow = performance.now();
    this.recentBeats = this.recentBeats.filter(t => densityNow - t < 2000);
    this.beatDensity = this.recentBeats.length / 2.0;
